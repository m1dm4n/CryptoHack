...
def inv_quarter_round(x, a, b, c, d):
    x[b] =rotate(x[b], 25)
    x[b] ^= x[c]
    x[c] = inv_word(x[c], x[d])
    x[d] = rotate(x[d], 24)
    x[d] ^= x[a]
    x[a] = inv_word(x[a], x[b])
    x[b]=rotate(x[b], 20)
    x[b] ^= x[c]
    x[c] = inv_word(x[c], x[d])
    x[d] = rotate(x[d], 16)
...
if __name__ == '__main__':
    ...
    print(key)
    c = ChaCha20()
    print(c.decrypt(msg_enc, key, iv1).decode('utf-8'))
    print(c.decrypt(flag_enc, key, iv2).decode('utf-8'))
